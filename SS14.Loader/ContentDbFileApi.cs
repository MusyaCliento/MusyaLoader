...
internal sealed class ContentDbFileApi : IFileApi, IDisposable
{
    ...
    public unsafe ContentDbFileApi(string contentDbPath, long version)
    {
        ...
        _dbConnections.Add(new ConPoolEntry(db, ZSTD_createDCtx(), InitBlob(db, initBlob)) { IsMaster = true });
        ...
        for (var i = 1; i < poolSize; i++)
        {
            ...
            CheckThrowSqliteErr(db, err);
            sqlite3_exec(db, "BEGIN");
            _dbConnections.Add(new ConPoolEntry(db, ZSTD_createDCtx(), InitBlob(db, initBlob)));
        }
    }

    private static sqlite3_blob InitBlob(sqlite3 db, long rowId)
    {
        var rc = sqlite3_blob_open(db, "main", "Content", "Data", rowId, 0, out var blob);
    ...
    public void Dispose()
    {
        ...
        for (var i = 0; i < _connectionPoolSize; i++)
        {
            ...
            if (!_dbConnections.TryTake(out var db)) continue;
    ...
    public bool TryOpen(string path, [NotNullWhen(true)] out Stream? stream)
    {
        ...
        try
        {
            if (!_dbConnections.TryTake(out entry))
                throw new InvalidOperationException("Failed to retrieve DB connection");
            ...
            switch (compression)
            {
                ...
                {
                    unsafe { ReadBlobZStd(buffer, blob, db, entry.DecompressionContext); }
        ...
        try
        {
            while (true)
            {
                ...
                fixed (byte* outputPtr = into)
                {
                    ...
                    {
                        if (into.Length != 0)
                            throw new InvalidOperationException("ZStd: Failed to fill buffer!");
                        return;
                    }
                }
            }
