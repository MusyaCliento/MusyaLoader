...
def publish_windows(x64_only: bool):
    ...
    dotnet_publish("SS14.ProxyService/SS14.ProxyService.csproj", "win-x64", False, "/p:FullRelease=True", "/p:RobustILLink=true")
    if os.name == 'nt':
        try:
            ...
            print(f"Bootstrap build failed (C++ tools missing?), skipping. Error: {e}")
            print("   Install 'Desktop Development with C++' workload in Visual Studio to enable Bootstrap builds.")
    ...
    if not x64_only:
        ...
        dotnet_publish("SS14.ProxyService/SS14.ProxyService.csproj", "win-arm64", False, "/p:FullRelease=True", "/p:RobustILLink=true")
    ...
    shutil.copytree(f"SS14.ProxyService/bin/Release/{TFM}/win-x64/publish", "bin/publish/Windows/bin_x64", dirs_exist_ok=True)
...
def publish_linux(x64_only: bool):
    ...
    dotnet_publish("SS14.ProxyService/SS14.ProxyService.csproj", "linux-x64", False, "/p:FullRelease=True", "/p:RobustILLink=true")
    ...
    if not x64_only:
        ...
        dotnet_publish("SS14.ProxyService/SS14.ProxyService.csproj", "linux-arm64", False, "/p:FullRelease=True", "/p:RobustILLink=true")
    ...
    shutil.copytree(f"SS14.ProxyService/bin/Release/{TFM}/linux-x64/publish", "bin/publish/Linux/bin_x64", dirs_exist_ok=True)
...
def publish_osx():
    ...
    shutil.copytree("PublishFiles/MusyaLoader.app", "bin/publish/macOS/MusyaLoader.app")

    res_root = "bin/publish/macOS/MusyaLoader.app/Contents/Resources"

    loader_res_root = f"{res_root}/Musya.app/Contents/Resources"
