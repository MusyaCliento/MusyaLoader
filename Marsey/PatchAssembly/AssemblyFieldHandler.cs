...
public static class AssemblyFieldHandler
{
    ...
    {
        string[] entryTypeCandidates = ["MarseyEntry", "ModEntry", "PatchEntry"];
        foreach (var typeName in entryTypeCandidates)
        {
            Type? entryType = assembly.GetType(typeName);
            MethodInfo? entry = entryType?.GetMethod("Entry", BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Static);
            if (entry != null)
                return entry;
        }

        return null; // This is going to be null if no supported entry type is present
    }
    ...
    public static void GetFields(
        Type? marseyPatchType,
        out string name,
        out string description,
        out string? iconResource)
    {
        ...
        iconResource = marseyPatchType?.GetField("IconResource")?.GetValue(null) as string;
    }
