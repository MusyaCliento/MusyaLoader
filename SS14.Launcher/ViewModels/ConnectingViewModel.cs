...
public class ConnectingViewModel : ViewModelBase
{
    ...
    public ConnectingViewModel(Connector connector, MainWindowViewModel windowVm, string? givenReason, ConnectionType connectionType)
    {
        ...
        this.WhenAnyValue(x => x._connector.LaunchFailureReason)
            .ObserveOn(RxApp.MainThreadScheduler)
            .Subscribe(_ =>
            {
                this.RaisePropertyChanged(nameof(StatusText));
    ...
    public string StatusText =>
        ...
        {
            Connector.ConnectionStatus.None => GetStatusTextOrRandomAction(_loc.GetString("connecting-status-none") + _reasonSuffix),
            Connector.ConnectionStatus.UpdateError => FormatUpdateError(),
            Connector.ConnectionStatus.Updating => GetStatusTextOrRandomAction(_loc.GetString("connecting-status-updating", ("status", _loc.GetString(_updaterStatus switch
            {
            ...
            Connector.ConnectionStatus.Connecting => GetStatusTextOrRandomAction(_loc.GetString("connecting-status-connecting") + _reasonSuffix),
            Connector.ConnectionStatus.ConnectionFailed => _loc.GetString("connecting-status-connection-failed") + FormatLaunchFailureReason(),
            Connector.ConnectionStatus.StartingClient => GetStatusTextOrRandomAction(_loc.GetString("connecting-status-starting-client") + _reasonSuffix),
    ...
    private string FormatLaunchFailureReason()
    {
        if (string.IsNullOrWhiteSpace(_connector.LaunchFailureReason))
            return "";

        return "\n" + _connector.LaunchFailureReason;
    }

    private string GetStatusTextOrRandomAction(string fallback)
    {
        if (!_cfg.GetCVar(CVars.RandConnAction))
            return fallback;

        _randomAction ??= TitleManager.RandAction();
