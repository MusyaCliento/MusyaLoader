...
internal static class Program
{
    ...
    public static void Main(string[] args)
    {
        ...
        var launcherLoggerConfig = new LoggerConfiguration()
        ...
        {
            launcherLoggerConfig = launcherLoggerConfig.WriteTo.File(
                LauncherPaths.PathLauncherLog,
                rollingInterval: RollingInterval.Day,
                retainedFileCountLimit: 7,
                fileSizeLimitBytes: 100L * 1024 * 1024);
        }

        Log.Logger = launcherLoggerConfig.CreateLogger();
    ...
    private static void TryApplyPendingUpdaterUpdate()
    {
        try
        {
            var updaterPath = GetUpdaterExecutablePathForCurrentBuild();
            if (string.IsNullOrWhiteSpace(updaterPath))
                return;

            var pending = updaterPath + ".pending";
            if (!File.Exists(pending))
                return;

            if (File.Exists(updaterPath))
                File.SetAttributes(updaterPath, FileAttributes.Normal);

            File.Copy(pending, updaterPath, true);
            File.Delete(pending);
            Log.Information("Applied pending updater self-update.");
        }
        catch (Exception e)
        {
            Log.Warning(e, "Failed to apply pending updater self-update.");
        }
    }

    private static string GetUpdaterExecutablePathForCurrentBuild()
    {
#if FULL_RELEASE
