...
public sealed class UrlFallbackSet
{
        ...
        CancellationToken cancel)
    {
        ...
        // Force fresh connection so proxy/profile switches apply immediately and stale proxy sockets aren't reused.
    ...
    private static bool ShouldDisableProxyForSession(Exception exception)
    {
        static bool MatchMessage(string text, string needle)
            => text.Contains(needle, StringComparison.OrdinalIgnoreCase);

        foreach (var ex in Flatten(exception))
        {
            var msg = ex.Message ?? string.Empty;
            if (MatchMessage(msg, "SOCKS server requested username & password authentication"))
                return true;
            if (MatchMessage(msg, "SOCKS5 requires username/password"))
                return true;
            if (MatchMessage(msg, "SOCKS5 auth failed"))
                return true;
        }
    ...
    {
        var stack = new System.Collections.Generic.Stack<Exception>();
