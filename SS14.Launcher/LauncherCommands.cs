...
public class LauncherCommands
{
    ...
    private static string? _lastConnectTarget;
    private static DateTime _lastConnectAtUtc;
    private static readonly TimeSpan ConnectDedupWindow = TimeSpan.FromMinutes(5);
    ...
    private async Task Connect(string param)
    {
        ...
        var normalizedTarget = NormalizeConnectTarget(param);
        var now = DateTime.UtcNow;

        if (_lastConnectTarget != null &&
            string.Equals(_lastConnectTarget, normalizedTarget, StringComparison.OrdinalIgnoreCase) &&
            now - _lastConnectAtUtc <= ConnectDedupWindow)
        {
            Log.Warning("Dropping duplicate connect command: {Target}", normalizedTarget);
            return;
        }
        ...
        _lastConnectTarget = normalizedTarget;
        _lastConnectAtUtc = now;
        ActivateWindow();
        Log.Information($"Connect command: \"{param}\", \"{reason}\"");
        ConnectingViewModel.StartConnect(_windowVm, param, reason);
    }

    private static string NormalizeConnectTarget(string param)
    {
    ...
    public const string DBGJumpCommand = ":Jumper";// Enable Marsey Debug Jumper
